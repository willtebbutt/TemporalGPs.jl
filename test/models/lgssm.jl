...
@testset "lgssm" begin
    ...
    @testset "rand_αs" begin
        rng = MersenneTwister(123456)
        N = 11
        Dlat = 3
        Dobs = 2
        S = 5
        @testset "$(storage_type)" for storage_type in [
            ArrayStorage(Float64),
            ArrayStorage(Float32),
            SArrayStorage(Float64),
            SArrayStorage(Float32),
        ]
            model = random_tv_lgssm(rng, Dlat, Dobs, N, storage_type)
            αs = TemporalGPs.rand_αs(rng, model, Val(S))
            @test αs isa Vector{<:AbstractMatrix}
            @test is_of_storage_type(αs[1], storage_type)
            @test length(αs) == N
            @test all(α -> size(α) == (Dobs, S), αs)
            @inferred TemporalGPs.rand_αs(rng, model, Val(S))
        end
    end
    ...
    @testset "correctness" begin
        ...
        @testset "(time_varying=$tv, Dlat=$Dlat, Dobs=$Dobs, $(storage.name)), N=$N" for
            ...
            ssm_interface_tests(
                ...
                check_adjoints=false, context=NoContext(),
            )
